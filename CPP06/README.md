...
This technique is called Exception Swallowing. usually, this is bad practice because it hides bugs.
	However, here it is the only way to check types sequentially using references.

dynamic_cast specifically looks at the vptr in the object's memory to reach the VTable negative offset,
	which points to the type_info structure defining the class's identity and inheritance history.
...
Q9: Is dynamic_cast fast? Should I use it in a high-performance game loop?
    ...
    | [8] member var_x  |         +---------------------+          +-----------------------+
    | [C] member var_y  |         | [ 0] ~Destructor()  |          | - Name: "1A"          |
    +-------------------+         | [ 1] virtual func() |          | - Base: &type_info_B  |
                                  +---------------------+          +-----------------------+


static_cast: The compiler calculates the offset at compile time. Runtime cost = 0.
...
When you enable RTTI (by adding a virtual function to the Base class), the compiler secretly attaches a
	Type Descriptor to every single object you create.

Without RTTI: The object is just a blob of data.
...
dynamic_cast (The Safe Question):
	...
	Abstract logic: "I know I am holding a Vehicle handle, but is the object actually a Ferrari?"

	Result: The system checks the ID card. If it matches (or inherits from) Ferrari,
	it gives you a valid pointer. If not, it returns NULL.

typeid (The Direct Look):

	bstract logic: "What is the exact name of the class of this object?"

	Result: It returns a unique type signature (e.g., "Class A"). Note: This was forbidden in your Ex02.
